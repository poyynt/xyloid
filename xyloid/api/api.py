...
def all_posts(page = 1):
	result = list(Posts.query.order_by(desc(Posts.internal_id)).paginate(page = page, per_page=10, max_per_page=10).items)
...
def posts_by_category(category, page = 1):
	...
	if category not in ("uncategorized"):
		result = list(Posts.query.filter((Posts.category1 == category) | (Posts.category2 == category) | (Posts.category3 == category) | (Posts.category4 == category) | (Posts.category5 == category)).order_by(desc(Posts.internal_id)).paginate(page=page, per_page=10, max_per_page=10).items)
	else:
		result = list(Posts.query.filter_by(category1="", category2="", category3="", category4="", category5="").order_by(desc(Posts.internal_id)).paginate(page=page, per_page=10, max_per_page=10).items)
