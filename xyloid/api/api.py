...
def all_posts(page=1):
    result = list(Posts.query.order_by(desc(Posts.internal_id)).paginate(
        page=page, per_page=10, max_per_page=10).items)
    result = [
        {
            "name": r.name,
            "uuid": r.uuid,
...
def posts_by_category(category, page=1):
    ...
    if category not in ("uncategorized"):
        result = list(Posts.query.filter((Posts.category1 == category) | (Posts.category2 == category) |
                                         (Posts.category3 == category) | (Posts.category4 == category) | (
                                                     Posts.category5 == category)).order_by(
            desc(Posts.internal_id)).paginate(page=page, per_page=10, max_per_page=10).items)
    else:
        result = list(
            Posts.query.filter_by(category1="", category2="", category3="", category4="", category5="").order_by(
                desc(Posts.internal_id)).paginate(page=page, per_page=10, max_per_page=10).items)
...
def category_count(category):
    if category not in ("uncategorized"):
        result = Posts.query.filter(
            (Posts.category1 == category) | (Posts.category2 == category) | (Posts.category3 == category) | (
                        Posts.category4 == category) | (Posts.category5 == category)).count()
    else:
...
def get_categories():
    result = Posts.query.with_entities(Posts.category1).union_all(Posts.query.with_entities(Posts.category2)).union_all(
        Posts.query.with_entities(Posts.category3)).union_all(Posts.query.with_entities(Posts.category4)).union_all(
        Posts.query.with_entities(Posts.category5)).filter(Posts.category1 != "").group_by(
        Posts.category1).with_entities(Posts.category1, func.count(Posts.category1)).order_by(
        desc(func.count(Posts.category1))).all()
    uncategorized = Posts.query.filter(
        (Posts.category1 == "") & (Posts.category2 == "") & (Posts.category3 == "") & (Posts.category4 == "") & (
                    Posts.category5 == "")).count()
